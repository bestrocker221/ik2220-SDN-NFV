...
if __name__ == "__main__":
	...
	for h in all_hosts_obj:
		...
		for test_ip in all_servers:
			...
			res = int(h.cmd('ping -c 1 -W 1 {}'.format(test_ip) + '| grep 64 | wc | awk \'{print $1}\''))
		...
		for lb_ip in [srv_list["web_lb"], srv_list["dns_lb"]]:
			...
			res = int(h.cmd('ping -c 1 -W 1 {}'.format(lb_ip) + '| grep 64 | wc | awk \'{print $1}\''))
			info( ok() if res == 1 else err() )

		### DNS TESTING
		for dns_ip in srv_list["dns"]:
			info("[DNS TEST] host {} to internal DNS {} must NOT work: ".format( h, dns_ip)),
			res = int(h.cmd('dig @{}'.format(dns_ip) + ' +timeout=1 +tries=1 ciao.it | grep "Got answer" |wc| awk \'{print $1}\''))
			info( ok() if res == 0 else err())

			# bogus DNS port
			info("[DNS BOGUS TEST] host {} to internal DNS port 5353 {} must NOT work: ".format( h, dns_ip)),
			res = int(h.cmd('dig -p 5353 @{}'.format(dns_ip) + ' +timeout=1 +tries=1 ciao.it | grep "Got answer" |wc| awk \'{print $1}\''))
			info( ok() if res == 0 else err() )

		# Testing DNS towards the virtual IP
		info("[DNS TEST] host {} to DNS LoadBalancer {} must work: ".format( h, srv_list["dns_lb"])),
		res = int(h.cmd('dig @{}'.format(srv_list["dns_lb"]) + ' +timeout=1 +tries=1 ciao.it | grep "Got answer" |wc| awk \'{print $1}\''))
		info( ok() if res == 1 else err())
		
		info("[DNS BOGUS TEST] internal {} to internal DNS LoadBalancer port 5353 {} must NOT work: ".format( h, srv_list["dns_lb"])),
		res = int(h.cmd('dig -p 5353 @{}'.format(srv_list["dns_lb"]) + ' +timeout=1 +tries=1 ciao.it | grep "Got answer" |wc| awk \'{print $1}\''))
		info( ok() if res == 0 else err() )
		...
		info("[WEB TEST] host {} to  WEB LoadBalancer {} must work: ".format( h, srv_list["web_lb"])),
		res = int(h.cmd('curl --connect-timeout 1 {}'.format(srv_list["web_lb"]) + ' -s | grep DOCTYPE | wc | awk \'{print $1}\''))
		info( ok() if res == 1 else err())
